...
export class SignupCompanyComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.validateForm = this.fb.group({
      ...
      companyName: [null, [Validators.required,  Validators.pattern(/^[a-zA-Z0-9&.\-\s]{2,}$/)]],
      adress: [null, [Validators.required, Validators.minLength(5)]],
      phone: [null, [Validators.pattern(/^\+?[0-9]{10,15}$/),Validators.required]],
      password: [null, [Validators.required,Validators.minLength(6) , Validators.pattern(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{6,}$/) ]],
